...
Displays detailed information about selected targets and ECM controls.
...
class TargetInspector(QWidget):
    ...
    def update_target(self, target_data: Optional[Dict[str, Any]]) -> None:
        ...
        # Match the engine's square-law Swerling detection model.
    ...
    def _calculate_pd(self, snr_db: float) -> float:
        """Calculate single-pulse Swerling-I Pd at the displayed SNR."""
        return calculate_pd_swerling(
            snr_db=snr_db, pfa=1e-6, swerling_case=1, n_pulses=1
        )
